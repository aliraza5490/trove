  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          "bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed left-1/2 top-[20%] sm:top-[25%] z-50 grid w-full max-w-lg -translate-x-1/2 rounded-xl border border-border/60 p-0 shadow-2xl transition-all duration-200",
          className
        )}
        {...props}
      >
...
function DialogFooter({
  ...
  return (
    <div
      ...
      className={cn(
        "flex flex-col-reverse sm:flex-row sm:justify-end sm:gap-2 p-4 border-t border-border/40",
        className
      )}
      {...props}
    />
